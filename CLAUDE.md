...
## コラム記事（src/data/post/）の execution ルール

コラム記事を新規作成・大幅改稿するときは、**必ず画像を生成して埋め込む**（文字だけの記事にしない）。ユーザーからの指示を待たない。

- 手順の正本は `/image-gen` スキルの「ぼんどコラム記事の画像生成（必須ルール）」セクション
- 1記事につき3枚（ヒーロー1＋図解2）／サイズ1600×896／保存先 `src/assets/images/column-{テーマslug}-{YYYY-MM}/`
- 既存コラムと絵柄を統一する（cream #F7F4EF ／ teal #3E8E8A ／ orange #E8622C ／ navy #22394D、文字は一切入れない）
- alt属性は内容を説明する日本語にする（SEO・アクセシビリティ要件）
- 公開前に `npm run build` を通し、`dist/client/news/{slug}/` の生成と強調表記の `<strong>` 化を確認する
